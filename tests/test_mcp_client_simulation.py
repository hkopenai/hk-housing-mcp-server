...
This module contains unit tests that simulate client interactions with an MCP server
running over HTTP, testing tool calls and responses.
"""
...
class TestMCPClientSimulation(unittest.TestCase):
    """
    Test class for simulating MCP client interactions with a live server.
    
    This class sets up a server subprocess for HTTP communication, runs tests
    to simulate client requests, and tears down the server after tests.
    """
    server_process = None
    ...
    def setUp(self):
        ...
        self.server_process = subprocess.Popen(
            [sys.executable, "-m", "hkopenai.hk_housing_mcp_server", "--sse"],
    ...
    def test_get_private_storage_tool(self):
        """
        Test the 'get_private_storage' tool by calling it with a specific year filter.
        
        This test ensures that the tool returns a valid response without errors.
        """
        logger.debug("Testing 'get_private_storage' tool...")
        asyncio.run(self._call_tool_and_assert("get_private_storage", {"year": 2023}))
